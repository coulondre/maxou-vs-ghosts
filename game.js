...
Bullet.prototype.checkhitEnemies = function() {
    for(var i = 0; i < enemies.length; i++) {
...
function checkKey(e, value) {
    ...
    if (keyID === 40) { //Up arrow
        player1.isDownKey = value;
        e.preventDefault();
    }
    if (keyID === 37) { //Up arrow
        player1.isLeftKey = value;
        e.preventDefault();
    }
    if (keyID === 32) { //Up arrow
...
function outOfBounds(a, x, y) {
    ...
    var newLeftX = x;
    var treeLineTop = 5;
    var treelLineBottom = 570;
    var treeLineRight = 750;
    var treeLineLeft = 65;

    return newBottomY > treelLineBottom || newTopY < treeLineTop || newRightX > treeLineRight || newLeftX < treeLineLeft
}
